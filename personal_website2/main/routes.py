...
def build_portfolio_header(p, ctrls):
    ...
    for p_type in p:
        ...
        if curr_idx + 1 <= ctrls.get('limits').get('portfolio'):
            ...
            if first_type:
                first_type = False
                header = header + f"""<a class="nav-link active" data-toggle="tab" href="#{p_type.get("href")}" role="tablist">"""
            else:
                header = header + f"""<a class="nav-link" data-toggle="tab" href="#{p_type.get("href")}" role="tablist">"""
            header = header + f"""<i class="{p_type.get("fa_style_prefix")} {p_type.get("fa_name")}" aria-hidden="true"></i></a></li>"""
    return header
...
def build_portfolio_tabs(p, ctrls):
    ...
    for p_type in p:
        if first_tab_pane:
            ...
            tabbed_pane = tabbed_pane + f"""<div class="tab-pane active" id="{p_type.get('href')}"><div class="ml-auto mr-auto"><div class="row">"""
        else:
            tabbed_pane = tabbed_pane + f"""<div class="tab-pane" id="{p_type.get('href')}"><div class="ml-auto mr-auto"><div class="row">"""
